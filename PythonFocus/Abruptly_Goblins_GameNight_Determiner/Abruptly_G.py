...
gamers = []
# Now we want to create a function that will update this list and
# add a new gamer to the this gamers list. Each gamer should be a
# dictionary with the following keys: "name": a string that contains
# the gamer's full or presumed name. E.g., "Vicky Very"
# "availability": a list of strings containing the names of the days
# of the week that the gamer is available. E.g., ["Monday", "Thursday", "Sunday"]

# Create a function called add_gamer that takes two parameters: gamer and gamers_list.
# The function should check that the argument passed to the gamer parameter has both "name" and a
# "availability" as keys and if so add gamer to gamers_list.
...
kimberly = {
    'name' : 'Kimberley Warner',
    'availability' : ['Monday', 'Tuesday', 'Friday']
}

add_gamer(kimberly, gamers)
print(gamers) #[{'name': 'Kimberley Warner', 'availability': ['Monday', 'Tuesday', 'Friday']}]
...
# correlates each day of the week with gamer availability.
#
# Instructions
# Create a function called build_daily_frequency_table that takes no argument returns a dictionary with
# the days of the week as keys and 0s for values. We'll be using this to count the availability per
...
# Lastly we need a way to pick the day with the most available people to attend
# so that we can schedule game night on that night.
#
# Write a function find_best_night that takes a dictionary availability_table and
# returns the key with the highest number.
...
print(game_night) #Thursday

# And let's make a list of all of the people who are available that night.
#
# Create a function available_on_night that takes two parameters: gamers_list and
# day and returns a list of people who are available on that particular day.
# Call available_on_night with gamers and game_night and save the result into the
...
# With the best day for Abruptly Goblins! determined with computer precision,
# we need to let the attendees know that the game night is on a night they can attend.
# Let's start by creating a form email to send to each of the participants that we'll
# fill out with data later.
#
# Define a string, called form_email with interpolation variables {name}, {day_of_week},
# and {game} (in case we decide we want to use this featureset to host a different game night).
# Use it to tell your gaming attendees the night their Abruptly Goblins! game can be played.

form_email = """ 
Dear {name},

The Sorcery Society is excited to announce we will be host a Game Night for {game} on {day_of_week} nights!
...
# Create a function send_email with three parameters: gamers_who_can_attend, day,
# and game. Print form_email for each gamer in gamers_who_can_attend with the
# appropriate day and game. Call send_email with attending_game_night, game_night,
# and "Abruptly Goblins!".
...
# Let's send out an email to everyone (whether they can attend the first night or not)
# whose marked themselves as available on our second game night.
